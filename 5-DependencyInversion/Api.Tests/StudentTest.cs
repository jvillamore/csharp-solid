...
public class StudentTest
{
    ...
    {
        var studentTest = new Student(1, "Pepito Pérez", new List<double>() { 3, 4.5 });
        var LogbookMock = new Mock<ILogbook>();
        var stundentRepositoryMock = new Mock<IStudentRepository>();
        stundentRepositoryMock.Setup(p => p.Add(studentTest)).Returns(studentTest);

        var studentController = new StudentController(stundentRepositoryMock.Object, LogbookMock.Object);
        var resultGetStudents = studentController.Add(studentTest);
        Console.WriteLine($"cantidad: {studentController.Get().Count()}");

        Assert.NotNull(resultGetStudents);
        Assert.Equal<Student>(studentTest, resultGetStudents);
    }
}